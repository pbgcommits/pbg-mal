...
public class stepA_mal {
    ...
    private final static String TRY_KW = "try*";
    private final static String CATCH_KW = "catch*";
    private final static Scanner s = new Scanner(System.in);
    public static void main(String args[]) {
        stepA_mal repl = new stepA_mal();
        Env env = new Env();
        Core core = new Core();
        ...
        env.set(new MalSymbol("readline"), new MalFunction() {
            @Override
            public MalType operate(MalType[] a) throws Exception {
                if (a.length > 0) {
        ...
        try {
            ...
            if (args.length > 0) {
                ...
                repl.repl("(println (str \"Mal [\" *host-language* \"]\"))", env);
            }
        } catch (Exception e) {
                ...
                switch (firstParam.toString()) {
                    ...
                    case FN_KW: {
                        ...
                        MalFunction f = new MalFunction() {
                            ...
                            public MalType operate(MalType[] a) throws Exception {
                                ...
                                return stepA_mal.this.eval(originalList.get(2), newEnv);
                            }
                        };
                    ...
                    case TRY_KW: {
                        ...
                        try {
                            ...
                            MalList catchBlock = (MalList) originalList.get(2);
                            if (catchBlock.size() < 3) {
                                throw new Exception("Expected 3 args to catch block, got " + catchBlock.size());
                            }
                            if (!catchBlock.get(0).equals(new MalSymbol(CATCH_KW))) {
                                throw new Exception(
                                    "Catch block should start with catch*; started with " + catchBlock.get(0)
                                );
                            }
                            Env newEnv = new Env(env);
